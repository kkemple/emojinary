...
const getRandomEmojinary = (team) => {
  return new Promise((res, rej) => {
    let teamJson = team.get('extra_data');
    let randoms = (teamJson && teamJson.randoms) ? teamJson.randoms : [];

    const fetchRandomEmojinaryCount = () => {
      return new RandomEmojinary().count();
    };

    const discoverUnsolvedId = (count) => {
      let haveMatch, randomId;

      while (!haveMatch) {
        randomId = Math.ceil(Math.random() * count);

        if(indexOf(randoms, randomId) === -1) {
          haveMatch = true;
        }
      }
    ...
    const fetchRandom = (randomId) => {
      return new RandomEmojinary({ id: randomId }).fetch();
    };

    const addRandomToTeamData = (random) => {
      randoms.push(random.get('id'));

      team.set({ extra_data: { randoms } }).save();

      return random;
    };

    const logRandomEmojinary = (random) => {
    ...
    fetchRandomEmojinaryCount()
      .then((count) => discoverUnsolvedId(count))
      .then((randomId) => fetchRandom(randomId))
      .then((random) => addRandomToTeamData(random))
      .then((random) => logRandomEmojinary(random))
      .then((random) => res(random))
      .catch((e) => {
  ...
  return new UserStats({ id: emojinary.user_id })
    ...
    .then((us) => {
      if (!us) {
        ...
        return us.save({ solves: (us.get('solves')) ? us.get('solves') + 1 : 1 });
      }
    })
    .catch((e) => {
  ...
  return new UserStats({ id: payload.user_id })
    ...
    .then((us) => {
      if (!us) {
        ...
        return us.save({ attempts: (us.get('attempts')) ? us.get('attempts') + 1 : 1 });
      }
    })
    .catch((e) => {
  ...
  return new UserStats({ id: payload.user_id })
    ...
    .then((us) => {
      if (!us) {
        ...
        return us.save({ forfeits: (us.get('forfeits')) ? us.get('forfeits') + 1 : 1 });
      }
    })
    .catch((e) => {
      logger.error('error updating user stats', {
...
export default {
  ...
  random (team) {
    return getRandomEmojinary(team);
  },
