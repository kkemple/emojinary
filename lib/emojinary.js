...
const lastSolvedPhrase = (model) => {
  ...
  The last one was solved by *${model.get('user_name')}*
    ...
    superagent
      ...
      .end((err) => {
        if (err) {
          ...
          res('Great job! You solved it! :tada: :trumpet: :confetti_ball:');
        }
      });
    });
};
      ...
      .then((model) => {
        ...
        return model
          ...
          .then((updatedModel) => {
            ...
            const message = `Another one solved! Good job ${solvedBy}! ${value}`;
