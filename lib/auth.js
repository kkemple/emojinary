...
const getTeamIfExists = Promise.method((identity) => {
  return new Promise((res, rej) => {
...
const createOrValidateTeam = Promise.method((opts) => {
  return (opts.team) ?
...
export default {
  ...
  login (authCode) {
    return getToken(authCode)
      ...
      .then((identity) => getUserMetaData(identity))
      .then((identity) => getTeamIfExists(identity))
      .then((opts) => createOrValidateTeam(opts))
